...
model.load_state_dict(model_data['model']['sd'])
model.eval()
...
csv_path = modelpath.with_name(modelpath.name + '-submission').with_suffix('.csv')
zip_path = modelpath.with_name(modelpath.name + '-submission').with_suffix('.zip')
